...
The cold-start portion is a separate concern from the response cap. [The FlashBoot mechanism investigation](/mineru-runpod/blog/2026-05-26-runpod-flashboot-mechanism-investigation/) covers why the ~112 s exists, how the boot-time warmup interacts with RunPod's snapshot system, and when subsequent cold starts are much faster.
...
If you're new to the template, the [getting-started guide](/mineru-runpod/getting-started/overview/) walks through the full deploy in about ten minutes. For the cold-start side of the picture (separate from the response cap covered here), see [the FlashBoot mechanism investigation](/mineru-runpod/blog/2026-05-26-runpod-flashboot-mechanism-investigation/). For GPU sizing, [Choosing a GPU](/mineru-runpod/guides/choosing-gpu/) covers when the default `ADA_24` (RTX 4090) is enough and when to opt up.
