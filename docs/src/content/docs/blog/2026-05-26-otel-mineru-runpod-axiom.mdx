...
**Region matters — use the right base URL.** Your workspace is in either the US region (`https://api.axiom.co`) or the EU region (`https://api.eu.axiom.co`). Check yours in the Axiom UI: **Settings → General** shows the workspace region. Using the wrong region for your workspace makes Axiom return `HTTP 400 {"code":400,"message":"mismatched region"}` on every export — and the OTel SDK logs only the HTTP code, not the message, so you'll just see endless `Failed to export ... code: 400` in the worker stdout with no clue why. This caught me on my own workspace and cost an hour of debugging.

**Base URL, not the full path.** Set the endpoint to the region root (`https://api.axiom.co` or `https://api.eu.axiom.co`), not `https://api.axiom.co/v1/traces`. The [OpenTelemetry Python SDK](https://opentelemetry-python.readthedocs.io/en/latest/exporter/otlp/otlp.html) appends `/v1/traces`, `/v1/logs`, `/v1/metrics` per signal automatically. If you set the full path, the SDK double-appends and Axiom returns 404.
...
- `Failed to export ... code: 400, reason: Bad Request` — the dataset resolves and auth works, but the payload is being rejected. **The #1 cause is region mismatch**: your workspace is in the EU but `OTEL_EXPORTER_OTLP_ENDPOINT` points at `https://api.axiom.co` (or vice versa). The SDK eats the response body so the worker stdout doesn't show *why* — confirm by running `curl -v POST https://api.axiom.co/v1/traces -H "Authorization: Bearer xaat-..." -H "x-axiom-dataset: <yours>" -H "Content-Type: application/x-protobuf" --data-binary "x"` against your endpoint; Axiom's response body will literally say `mismatched region` if that's the cause. Other 400 causes: wrong header name on metrics (use `x-axiom-metrics-dataset`, not `x-axiom-dataset`), or the dataset was created of the wrong type for the signal.
