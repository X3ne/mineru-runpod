...
| `lang` | string | `"en"` | Language hint, **pipeline backend only** (VLM backends ignore it). Use script-family codes — `east_slavic`, `cyrillic`, `latin`, `arabic`, `devanagari`, `japan`, `korean`, `chinese_cht`, `el`, `th`, etc. 109 languages supported. NOT ISO codes |
...
| `model_dir` | Filesystem path of the model snapshot that loaded. Useful for verifying that a Cached Models endpoint is hitting the right snapshot (`null` if HF_HOME is empty) |
