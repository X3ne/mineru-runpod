---
title: Clients
description: Two ways to call your runpod-mineru endpoint. The Python MineruClient for quick starts, and the direct RunPod SDK / HTTP API for production.
---
...
- **No typed response models.** Returns a raw `dict`. If you want a `ParsedDocument` Pydantic model, write your own adapter (see [the adapter example](https://github.com/sergeyshmakov/runpod-mineru/blob/main/examples/parser_adapter_example.py)).
...
Full job-input shape is in [API reference](/runpod-mineru/reference/api/).
...
Failure responses set `ok: false` and include a top-level `error` key; RunPod marks the job FAILED in the dashboard. See [API reference](/runpod-mineru/reference/api/) for the full schema.
