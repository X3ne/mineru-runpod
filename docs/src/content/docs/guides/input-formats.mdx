...
For files up to 200 MB, a signed S3 URL via `file_url` is cheapest; for larger books or image archives, pre-stage on a network volume and use `volume_path` — see [Network volumes](/mineru-runpod/guides/network-volumes/).
