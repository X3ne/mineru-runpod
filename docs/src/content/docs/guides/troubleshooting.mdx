...
| `phase_ms.mineru_parse` | Per-page guidance for warm workers, **highly GPU-dependent**: MinerU upstream cites **~0.5 s/page (2.12 fps) on an A100** for the VLM backend; we measured **~3.5 s/page on an A5000 24 GB** under the default `gpu_memory_utilization=0.5`. Pipeline backend is ~3–5 s/page across GPUs (CPU-bound for layout, GPU-bound only for OCR). **First call on a fresh worker is much higher** (model load + vLLM warmup adds ~90–130 s for the VLM backend). If a warm-worker call is 5× the expected per-page number for your GPU, you're memory-bound and vLLM is swapping |
...
| Warm worker, VLM, on A5000 24 GB (our measurement) | ~3.5 s/page |
...
- **Per-page math (warm worker, GPU-dependent)**: MinerU upstream cites ~0.5 s/page for the VLM backend on an A100. We measured ~3.5 s/page on an A5000 24 GB. Pipeline ≈ 3–5 s/page across GPUs. A 1000-page book on pipeline = ~3000–5000 s; on VLM-on-A5000 ≈ ~3500 s; on VLM-on-A100 ≈ ~500 s. **Add 90–130 s for the first call on a cold worker if VLM** (model load + vLLM warmup) — the cold-start tax is paid once per worker, not per page.
